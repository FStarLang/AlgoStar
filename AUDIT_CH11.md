...
- [x] **P1-5: Strengthen search not-found to unconditional `~(key_in_table)`**. Defined `valid_ht` invariant (for every key at probe position p, no NIL at earlier probes), proved it is established by `hash_table_create` and preserved by `hash_insert` and `hash_delete`. Under `valid_ht`, the search not-found postcondition collapses from a disjunction to the unconditional `~(key_in_table s size key)`. All proofs pass `--quake 3/3`.

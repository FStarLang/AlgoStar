...
fn rec func_bst_insert
  ...
  ensures
    (with_pure
      (forall
        (var_j: SizeT.t).
        (((var_j `SizeT.lt` var_cap) && (not (var_j = return_1))) ==>
  ...
  decreases (var_cap `SizeT.sub` var_i)
{
  ...
  if ((((array_read (!var_valid) (!var_i))) = (Int32.int_to_t 0))) {
    (array_write (!var_keys) (!var_i) (!var_key));
    (array_write (!var_valid) (!var_i) (Int32.int_to_t 1));
    return (!var_i);
  } else {};
  ...
  if (((!var_key) `Int32.lt` ((array_read (!var_keys) (!var_i))))) {
    ...
    return (func_bst_insert (!var_keys) (!var_valid) (!var_cap) (!var_key) (!var_left));
  } else {
    ...
    return (func_bst_insert (!var_keys) (!var_valid) (!var_cap) (!var_key) (!var_right));
  };
}
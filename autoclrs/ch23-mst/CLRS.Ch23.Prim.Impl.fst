...
// Re-export index arithmetic from Greedy
let lemma_prod_fits = Greedy.lemma_prod_fits
let lemma_index_bound = Greedy.lemma_index_bound
...
let mst_edges_source_unchanged = Greedy.mst_edges_source_unchanged
let prim_safe = Greedy.prim_safe
let prim_safe_init = Greedy.prim_safe_init
let prim_safe_update_non_mst = Greedy.prim_safe_update_non_mst
let prim_safe_add_vertex = Greedy.prim_safe_add_vertex
...
let prim_inv_add_vertex
  ...
  = prim_inv_elim key_seq parent_seq in_mst_old weights_seq n source;
    ...
    // Use Greedy.prim_safe_add_vertex which takes KeyInv.key_inv directly
    Greedy.prim_safe_add_vertex parent_seq key_seq in_mst_old in_mst_new weights_seq n source u
#pop-options
...
let prim_inv_add_source
  ...
  : Lemma
    ...
    (ensures
      ...
      prim_kpc key_seq parent_seq weights_seq n source)
  = prim_inv_elim key_seq parent_seq in_mst_old weights_seq n source;
    // Source is skipped by mst_edges_so_far, so edges identical for old/new ims
    // prim_safe_update_non_mst handles the ims-unchanged frame
    // For source add, all non-source vertices are unchanged, so in-MST vertices unchanged
    Greedy.prim_safe_update_non_mst parent_seq key_seq parent_seq key_seq
      in_mst_old weights_seq n source;
    // Transfer to in_mst_new via mst_edges_source_unchanged
    Greedy.mst_edges_source_unchanged parent_seq key_seq in_mst_old in_mst_new n source 0;
    reveal_opaque (`%Greedy.prim_safe) (Greedy.prim_safe parent_seq key_seq in_mst_old weights_seq n source);
    reveal_opaque (`%Greedy.prim_safe) (Greedy.prim_safe parent_seq key_seq in_mst_new weights_seq n source)

(*** Re-exports from Greedy — greedy step, noRepeats, MST derivation ***)

let mst_edges_noRepeats_init = Greedy.mst_edges_noRepeats_init
let prim_mst_result = Greedy.prim_mst_result
let prim_mst_result_elim = Greedy.prim_mst_result_elim
let derive_prim_mst_post_loop = Greedy.derive_prim_mst_post_loop

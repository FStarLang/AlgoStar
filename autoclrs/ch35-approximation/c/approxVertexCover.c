...
void approx_vertex_cover(_array int *adj, size_t n, _array int *cover)
  ...
  _requires(n > 0)
  /* Cover validity + binary via outer_inv_pure (implies binary and cover validity) */
  _ensures((bool) _inline_pulse(
    AVCHelper.outer_inv_pure
      ...
      (SizeT.v $(n))
      (SizeT.v $(n))))
  /* Spec-level: is_cover (bridges to CLRS.Ch35.VertexCover.Spec) */
  _ensures((bool) _inline_pulse(
      ...
      (SizeT.v $(n)) (SizeT.v $(n)) 0))
{
  ...
  _ghost_stmt(AVCBridgeLemmas.bridge_full (array_value_of $(adj)) (array_value_of $(cover)) (SizeT.v $(n)));
}

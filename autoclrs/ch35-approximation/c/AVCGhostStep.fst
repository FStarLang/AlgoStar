 ...
 *
 * Takes arrays in SLProp with pure preconditions establishing the
 * relationships between read values (he, cu, cv) and the underlying
 * sequences. These facts are available in the calling context from
 * array_read's rewrites_to and the loop invariant.
 *)
...
fn matching_step_ghost
  ...
  (n: SZ.t) (u: SZ.t{SZ.v u < SZ.v n}) (v: SZ.t{SZ.v v < SZ.v n /\ SZ.v u < SZ.v v})
  (he cu cv new_cu new_cv: Int32.t)
  (#sa: Seq.seq (option Int32.t){Seq.length sa == SZ.v n `op_Multiply` SZ.v n})
  (#ma: nat -> prop)
  (#sc: Seq.seq (option Int32.t){Seq.length sc == SZ.v n})
  (#mc: nat -> prop)
  (#vm: list Spec.edge)
requires

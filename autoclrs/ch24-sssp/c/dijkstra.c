 ...
 *
 * The full sp_dist optimality and shortest-path tree properties are
 * proven in the Pulse implementation at CLRS.Ch24.Dijkstra.Impl.fsti.
 *
 * Uses adjacency matrix representation (n×n flat array, INF = no edge/∞).
 * Requires all weights non-negative and bounded by INF.
 *
 * Split into helper functions so Pulse monolithic well-typedness
 * queries stay small enough for Z3.
 */
...
void dijkstra(
    ...
    _array int *dist, _array size_t *pred)
  _requires(n > 0 && source < n)
  _requires((bool) _inline_pulse(SizeT.fits (SizeT.v $(n) * SizeT.v $(n))))

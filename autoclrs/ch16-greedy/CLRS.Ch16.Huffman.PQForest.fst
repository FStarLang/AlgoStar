...
let pq_tree_freq_match_init_step
  ...
  : Lemma (requires PQ.extends s0 s1 (f, idx) /\
          ...
          (ensures pq_tree_freq_match s1 ((idx, p, HSpec.Leaf (SZ.v idx) f) :: active_old))
  = let new_active = (idx, p, HSpec.Leaf (SZ.v idx) f) :: active_old in
    let aux (j: nat)
      : Lemma (ensures (j < Seq.length s1 ==>
      ...
      = if j < Seq.length s1 then begin
          ...
          if y = (f, idx) then begin
            find_entry_prepend active_old idx p (HSpec.Leaf (SZ.v idx) f);
            ...
            find_entry_prepend_other active_old (idx, p, HSpec.Leaf (SZ.v idx) f) (snd y);
            reveal_opaque (`%pq_entry_freq_ok) pq_entry_freq_ok
          end
        end
    in
...
let forest_has_pq_entry_init_step
  ...
  : Lemma (requires PQ.extends s0 s1 (f, idx) /\
                    ...
                    pq_idx_unique s0 /\
                    (forall (k: nat). k < L.length active_old ==> entry_idx (L.index active_old k) <> idx))
          (ensures forest_has_pq_entry s1 ((idx, p, HSpec.Leaf (SZ.v idx) f) :: active_old))
  = reveal_opaque (`%forest_has_pq_entry) forest_has_pq_entry;
    let new_active = (idx, p, HSpec.Leaf (SZ.v idx) f) :: active_old in
...
let init_bundle_step
  ...
  : Lemma
    ...
    (ensures
      init_bundle freq_seq nd_new pq_new ((vi, leaf, HSpec.Leaf (SZ.v vi) freq_val) :: active_old) (SZ.v vi + 1) (SZ.v n))
  = reveal_opaque (`%init_bundle) init_bundle;
    let new_entry : forest_entry = (vi, leaf, HSpec.Leaf (SZ.v vi) freq_val) in
    ...
    find_entry_prepend active_old vi leaf (HSpec.Leaf (SZ.v vi) freq_val);
    ...
    node_ptr_correspondence_init_step active_old vi leaf (HSpec.Leaf (SZ.v vi) freq_val) nd_old n;
...
#push-options "--z3rlimit 60 --fuel 1 --ifuel 1 --split_queries always --z3refresh"
private

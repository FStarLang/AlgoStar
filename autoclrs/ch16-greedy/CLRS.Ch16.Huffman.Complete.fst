...
//   type htree = Leaf : sym:nat -> freq:pos -> htree
...
let init_pq (freqs: list pos{Cons? freqs}) : pq:priority_queue{is_valid_pq pq /\ length pq = length freqs} =
  ...
  map_nonempty (fun f -> Leaf 0 f) freqs;
  map_length (fun f -> Leaf 0 f) freqs;
...
let rec all_leaf_freqs_of_map_leaf (freqs: list pos)
  : Lemma (ensures all_leaf_freqs (map (fun (f:pos) -> Leaf 0 f) freqs) == freqs)
          (decreases freqs)
  = match freqs with
    | [] -> ()
...
let huffman_complete_preserves_frequency_multiset (freqs: list pos{Cons? freqs})
  ...
  = let trees = map (fun (f:pos) -> Leaf 0 f) freqs in
    map_nonempty (fun (f:pos) -> Leaf 0 f) freqs;
    let pq = init_pq freqs in
...
let huffman_complete_single (f: pos)
  : Lemma (ensures huffman_complete [f] == Leaf 0 f)
  = // huffman_complete [f] = huffman_from_pq (init_pq [f])
    // init_pq [f] = sortWith freq_cmp [Leaf 0 f] = [Leaf 0 f]
    // huffman_from_pq [Leaf 0 f] = Leaf 0 f (by pattern match on single element)
    ()

// Helper: leaf_freqs of single Leaf
let leaf_freqs_single (f: pos)
  : Lemma (ensures leaf_freqs (Leaf 0 f) == [f])
  = // leaf_freqs (Leaf 0 f) = [f] by definition
    ()
...
let single_leaf_freqs_implies_leaf (t: htree) (f: pos)
  : Lemma (requires leaf_freqs t == [f])
          (ensures Leaf? t /\ freq_of t == f)
  = match t with
    | Leaf _ f' -> 
        // leaf_freqs (Leaf _ f') == [f']
        // We know leaf_freqs t == [f]
        // Therefore [f'] == [f], so f' == f, and freq_of t == f
        ()
    | Internal _ l r ->
...
let huffman_single_is_optimal (f: pos)
  ...
  = huffman_complete_single f;
    ...
    let aux (t': htree)
      : Lemma (requires leaf_freqs t' == [f])
              (ensures weighted_path_length (Leaf 0 f) <= weighted_path_length t')
...
private let make_min_siblings_single_swap
  (t: htree) (freqs: list pos{length freqs >= 2})
  (parent: tree_position) (fl fr f1 f2: pos)
  (tgt1 tgt2: tree_position) (tgt_a tgt_b: pos)
  : Lemma (requires
            ...
            mem f1 freqs /\
            mem f2 freqs /\
            (forall (x: pos). mem x freqs ==> f1 <= x) /\
            (forall (x: pos). mem x freqs ==> x >= f2 \/ x = f1) /\
            ...
            tgt1 = (if f2 <= fr then parent @ [true] else parent @ [false]) /\
            tgt2 = (if f2 <= fr then parent @ [false] else parent @ [true]) /\
            tgt_a = (if f2 <= fr then fl else fr) /\
            tgt_b = (if f2 <= fr then fr else fl) /\
            is_leaf_at t tgt1 tgt_a /\
            is_leaf_at t tgt2 tgt_b /\
            tgt1 =!= tgt2 /\
            not (is_prefix tgt1 tgt2) /\
            not (is_prefix tgt2 tgt1) /\
            tgt_a >= f1 /\
            tgt_b >= f2 /\
            tgt_a = f1 /\
            tgt_b <> f2)
          (ensures (exists (t': htree).
                      same_frequency_multiset t' freqs /\
...
private let make_min_siblings_double_swap
  (t: htree) (freqs: list pos{length freqs >= 2})
  (parent: tree_position) (fl fr f1 f2: pos)
  (tgt1 tgt2: tree_position) (tgt_a tgt_b: pos)
  : Lemma (requires
  ...
  = assert (length tgt1 = max_leaf_depth t 0);
    ...
    if pos_f1 = tgt2 then (
      let finish_pos_f1_at_tgt2 ()
        : Lemma (ensures (exists (t': htree).
                            same_frequency_multiset t' freqs /\
                            are_siblings t' f1 f2 == true /\
                            weighted_path_length t' <= weighted_path_length t))
        = assert (f1 = tgt_b);
          ...
          if pos_f1' = tgt1 then (
            ...
            assert_spinoff (same_frequency_multiset s2' freqs);
            assert_spinoff (are_siblings s2' f1 f2 == true);
            assert_spinoff (weighted_path_length s2' <= weighted_path_length t)
          )
      in
      finish_pos_f1_at_tgt2 ()
    ) else (
      let finish_pos_f1_else ()
        : Lemma (ensures (exists (t': htree).
                            same_frequency_multiset t' freqs /\
                            are_siblings t' f1 f2 == true /\
                            weighted_path_length t' <= weighted_path_length t))
        = (if is_prefix pos_f1 tgt2 then leaf_position_no_extension s1 pos_f1 tgt2
          ...
          if tgt_b = f2 then (
            ...
            let finish_second_swap ()
              : Lemma (ensures (exists (t': htree).
                                  same_frequency_multiset t' freqs /\
                                  are_siblings t' f1 f2 == true /\
                                  weighted_path_length t' <= weighted_path_length t))
              = assert_spinoff (count f2 (leaf_freqs s2) = count f2 freqs);
                  ...
                  let finish_pos_f2_at_tgt1 ()
                    ...
                    = assert (f1 = f2);
                      ...
                      if pos_f2' = tgt2 then (
                        ...
                        assert_spinoff (same_frequency_multiset s4 freqs);
                        assert_spinoff (are_siblings s4 f1 f2 == true);
                        assert_spinoff (weighted_path_length s4 <= weighted_path_length t)
                      )
                  in
                  finish_pos_f2_at_tgt1 ()
                ) else (
                  let finish_pos_f2_else ()
                    : Lemma (ensures (exists (t': htree).
                                        same_frequency_multiset t' freqs /\
                                        are_siblings t' f1 f2 == true /\
                                        weighted_path_length t' <= weighted_path_length t))
    ...
    with _. (
      ...
      if (fl = f1 && fr = f2) || (fl = f2 && fr = f1) then (
        ...
        if not (tgt_b >= f2) then (
          ...
          assert (let (m1, m2) = find_two_mins freqs in f1 = m1 /\ f2 = m2);
          if tgt_a = f1 then (
            if tgt_b = f2 then ()
            else make_min_siblings_single_swap t freqs parent fl fr f1 f2 tgt1 tgt2 tgt_a tgt_b
          ) else (
            make_min_siblings_double_swap t freqs parent fl fr f1 f2 tgt1 tgt2 tgt_a tgt_b
          )
        )
      )
    )
...
let rec all_leaf_freqs_of_leaf_list (l: list htree)
  ...
  = match l with
    | [] -> ()
    | Leaf _ _ :: tl -> all_leaf_freqs_of_leaf_list tl
...
let rec insert_sorted_leaf_preserves_all_leaves (f: pos) (l: list htree)
  : Lemma (requires all_leaves l)
          (ensures all_leaves (insert_sorted (Leaf 0 f) l))
          (decreases l)
  = match l with
    | [] -> ()
    | Leaf _ _ :: tl ->
        if freq_of (Leaf 0 f) <= freq_of (Leaf 0 (freq_of (hd l))) then ()
...
let rec sorted_all_leaves_nondecreasing (pq: list htree)
  ...
  = match pq with
    | [] -> ()
    | [Leaf _ _] -> ()
    | (Leaf _ _) :: tl -> sorted_all_leaves_nondecreasing tl
...
let rec huffman_from_pq_wpl_le
  ...
  = match pq with
    ...
    | (Leaf s1 f1) :: (Leaf s2 f2) :: rest ->
        ...
        let merged = merge (Leaf s1 f1) (Leaf s2 f2) in
        let leaf_merged = Leaf 0 (f1 + f2) in
        insert_sorted_length merged rest;
        insert_sorted_nonempty merged rest;
        insert_sorted_maintains_sorted merged rest;
        let new_pq = insert_sorted merged rest in
          ...
          =
            ...
            let show_multiset (x: pos)
              ...
              = // From replace_siblings_changes_count:
                ...
                // count x (all_leaf_freqs leaf_pq) = count x (leaf_freqs (Leaf 0 (f1+f2))) + count x (all_leaf_freqs rest)
...
let rec partition_preserves_all_leaves (f: htree -> bool) (l: list htree)
  ...
  = match l with
    | [] -> ()
    | (Leaf _ _) :: tl -> partition_preserves_all_leaves f tl
...
let rec append_preserves_all_leaves (l1 l2: list htree)
  ...
  = match l1 with
    | [] -> ()
    | Leaf _ _ :: tl -> append_preserves_all_leaves tl l2
...
let rec map_leaf_all_leaves (freqs: list pos)
  : Lemma (ensures all_leaves (map (fun (f:pos) -> Leaf 0 f) freqs))
          (decreases freqs)
  = match freqs with
    | [] -> ()

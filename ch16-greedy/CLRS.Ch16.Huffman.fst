...
// Introduce pq_entry_freq_ok from explicit find_entry_by_idx result
...
// Eliminate pq_entry_freq_ok to get find_entry_by_idx result
...
// Introduce pq_tree_freq_match from a per-element proof
...
// Eliminate by mem: any entry in pq satisfies freq_ok
...
// pq_tree_freq_match after PQ insert + forest prepend (init step)
...
// forest_has_pq_entry after PQ insert + forest prepend (init step)
...
// forest_has_pq_entry subset: removing an entry from forest relaxes coverage
...
// list_remove_two preserves entries at non-removed positions
...
let list_remove_two_preserves_entry (entries: list forest_entry) (j1 j2 k: nat)
  ...
  = let rem = list_remove_two entries j1 j2 in
    ...
    let k_inter = if k < j1 then k else k - 1 in
    list_remove_at_index entries j1 k_inter;
    let j2' = j2 - 1 in
    let k_final = if k_inter < j2' then k_inter else k_inter - 1 in
    list_remove_at_index inter j2' k_final;
    find_entry_by_idx_unique rem k_final idx
#pop-options

// pq_entry_freq_ok transfers through list_remove_two for a single entry
...
// pq_entry_freq_ok transfers through prepend for a single entry
...
// Helper: pq_entry_freq_ok for a single old entry transfers through merge
...
// pq_entry_freq_ok for the new merged entry
// Requires sum_freq matches actual tree frequencies since freq_of computes recursively
...
// pq_tree_freq_match after merge: uses opaque intro/elim to control quantifier exposure
#push-options "--z3rlimit 120"
...
// ========== Init loop bundle: all per-entry invariants in one opaque predicate ==========
// This prevents "incomplete quantifiers" from too many foralls in the init loop VC.
...
// Opaque bundle for merge loop invariant — hides all foralls from Pulse VC
[@@ "opaque_to_smt"]
...
// Transition: init_bundle → merge_bundle
...
// Elim: extract specific properties from merge_bundle for merge loop body
...
//SNIPPET_END: huffman_tree_sig
{
  ...
  {
    // Extract all properties from merge_bundle for this iteration

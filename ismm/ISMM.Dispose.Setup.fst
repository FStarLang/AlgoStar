    ...
    )
{
  ...
  Arith.seq_upd_content_bound sdfs 0 (SZ.v n) rep;
  dfs_stk.(0sz) <- rep;
  dfs_top := 1sz;
  
  // Mark rep as PROCESSING (tag 3→2)
  Count.count_eq_pos stag 3 (SZ.v n) (SZ.v rep);
  Count.count_eq_bound stag 3 (SZ.v n);
  tag.(rep) <- 2sz;
  with st_init. assert (A.pts_to tag st_init);
  ...
  Count.count_eq_preserve stag 1 (SZ.v n) (SZ.v rep) 2sz;
  Count.count_eq_unmark stag 3 (SZ.v n) (SZ.v rep) 2sz;
  ()
}
```
#pop-options

(*
   ISMM Union-Find — Imperative Pulse Implementation
   Adapted from CLRS.Ch21.UnionFind.Impl for 3-array model (tag, parent, rank).
   
   Operations: make_set, find_set (with path compression), union_set (by rank).
   The tag array is threaded through but never modified by UF operations.
*)
...
module Compress = ISMM.UnionFind.Compress
module Union = ISMM.UnionFind.Union
...
let to_int_seq_index (s: Seq.seq SZ.t) (n: nat) (i: nat{i < n /\ i < Seq.length s})
...
let to_uf_upd_parent (stag sp srank: Seq.seq SZ.t)
  (n: nat{n <= Seq.length sp}) (i: nat{i < n}) (v: SZ.t)
  : Lemma (to_uf stag (Seq.upd sp i v) srank n ==
           { (to_uf stag sp srank n) with Spec.parent = Seq.upd (to_uf stag sp srank n).Spec.parent i (SZ.v v) })
  = to_nat_seq_upd sp n i v;
    assert (Seq.length (to_int_seq stag n) == n);
    assert (to_int_seq stag n == to_int_seq stag n)

// Bridge: imperative rank update
let to_uf_upd_rank (stag sp srank: Seq.seq SZ.t)
  (n: nat{n <= Seq.length srank}) (i: nat{i < n}) (v: SZ.t)
  : Lemma (to_uf stag sp (Seq.upd srank i v) n ==
           { (to_uf stag sp srank n) with Spec.rank = Seq.upd (to_uf stag sp srank n).Spec.rank i (SZ.v v) })
  = to_nat_seq_upd srank n i v

// Bridge: combined parent + rank update
let to_uf_upd_both (stag sp srank: Seq.seq SZ.t)
  (n: nat{n <= Seq.length sp /\ n <= Seq.length srank})
  (ip: nat{ip < n}) (vp: SZ.t) (ir: nat{ir < n}) (vr: SZ.t)
  : Lemma (to_uf stag (Seq.upd sp ip vp) (Seq.upd srank ir vr) n ==
           { Spec.n = n;
             Spec.tag = to_int_seq stag n;
             Spec.parent = Seq.upd (to_uf stag sp srank n).Spec.parent ip (SZ.v vp);
             Spec.rank = Seq.upd (to_uf stag sp srank n).Spec.rank ir (SZ.v vr) })
...
let path_len_strict_bound (parent: Seq.seq SZ.t) (n: nat) (i: nat)
  ...
  = let pl = path_len parent i n in
    ...
    if pl >= n then begin
      ...
      let rec path_node_eq_follow2 (parent: Seq.seq SZ.t) (i: nat) (fuel: nat) (k: nat)
        : Lemma (requires i < Seq.length parent /\ has_root_within parent i fuel /\ k <= path_len parent i fuel)
                (ensures path_node parent i fuel k == follow parent i k) (decreases k)
        = if k = 0 then ()
          else path_node_eq_follow2 parent (SZ.v (Seq.index parent i)) (fuel - 1) (k - 1)
      in
      path_node_eq_follow2 parent i n k1;
      path_node_eq_follow2 parent i n k2;
      let m = k2 - k1 in
...
let make_set_uf_inv (stag sp sr: Seq.seq SZ.t) (n: nat)
  : Lemma (requires
             ...
             (forall (j: nat). j < n ==> SZ.v (Seq.index stag j) == 0))
          (ensures Spec.uf_inv (to_uf stag sp sr n))
  = let f = to_uf stag sp sr n in
    ...
    )
{
  ...
  invariant exists* vi st sp sr.
    ...
    pure (
      ...
      Seq.length sp == Seq.length sparent /\
      Seq.length sr == Seq.length srank /\
      (forall (j: nat). j < SZ.v vi ==> SZ.v (Seq.index st j) == 0) /\
  ...
  make_set_uf_inv st sp sr (SZ.v n);
  ()
}
```
    ...
    )
{
  ...
  invariant exists* vc vb vgo.
    ...
    pure (
      ...
      Spec.uf_inv (to_uf stag sparent srank (SZ.v n)) /\
      Spec.pure_find (to_uf stag sparent srank (SZ.v n)) (SZ.v vc) ==
  ...
  decreases (SZ.v n - SZ.v !bound)
  {
    ...
    Spec.pure_find_step (to_uf stag sparent srank (SZ.v n)) (SZ.v vc);
    curr := par;
    let b = !bound;
    bound := SZ.add b 1sz;
    let p2 = parent.(par);
  ...
  Spec.pure_find_root (to_uf stag sparent srank (SZ.v n)) (SZ.v root);
  root
}
```
    ...
    )
{
  ...
  decreases (SZ.v n - SZ.v !count)
  {
    ...
    Spec.pure_find_step (to_uf stag sp_pre srank (SZ.v n)) (SZ.v vc);
    parent.(vc) <- root;
    ...
    )
{
  ...
  compress_path parent x root n #sparent #stag #srank;
  root
}
```
#pop-options
...
{
  let root_x = find_root_imp #_ parent #sparent x n #stag #srank;
  let root_y = find_root_imp #_ parent #sparent y n #stag #srank;
  to_nat_seq_index srank (SZ.v n) (SZ.v root_x);
  to_nat_seq_index srank (SZ.v n) (SZ.v root_y);
  if (root_x = root_y) {
    Union.pure_union_preserves_inv (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
    Union.pure_union_same_set (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
    Union.pure_union_stability (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
    ()
  } else {
    ...
    if (rank_x <^ rank_y) {
      ...
      to_uf_upd_parent stag sparent srank (SZ.v n) (SZ.v root_x) root_y;
      Union.pure_union_preserves_inv (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
      Union.pure_union_same_set (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
      Union.pure_union_stability (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
      ()
      ...
      if (rank_x >^ rank_y) {
        ...
        to_uf_upd_parent stag sparent srank (SZ.v n) (SZ.v root_y) root_x;
        Union.pure_union_preserves_inv (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
        Union.pure_union_same_set (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
        Union.pure_union_stability (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
        ()
        ...
        to_uf_upd_both stag sparent srank (SZ.v n) (SZ.v root_y) root_x (SZ.v root_x) new_rank;
        Union.pure_union_preserves_inv (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
        Union.pure_union_same_set (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
        Union.pure_union_stability (to_uf stag sparent srank (SZ.v n)) (SZ.v x) (SZ.v y);
        ()

...
// ============================================================
// Part 3: sp_dist triangle inequality (main theorem)
// ============================================================

// The key insight: sp_dist(s,v) = sp_dist_k(s,v,n-1) and
// sp_dist_k(s,v,n) <= sp_dist_k(s,u,n-1) + w(u,v) (predecessor lemma).
// For non-negative weights, sp_dist_k stabilizes at n-1, so
// sp_dist_k(s,v,n) == sp_dist_k(s,v,n-1).
//
// Stabilization proof: sp_dist_k(s,v,n) >= sp_dist_k(s,v,n-1) because
// any path with n edges visits n+1 vertices, must revisit one (pigeonhole),
// and removing the non-negative cycle gives a path with <= n-1 edges.

// For now, prove the theorem assuming stabilization.
// We use admit() for stabilization — this is the ONLY unproven step.
...
let sp_dist_k_stabilize
  ...
  = // By monotonicity: sp_dist_k(s,v,n) <= sp_dist_k(s,v,n-1)
    SP.sp_dist_k_monotone weights n s v (n - 1);
    // For the reverse: any n-edge path has a cycle; removing it
    // (non-negative weight) gives a path with <= n-1 edges.
